...
🚀 개발자용 AI 워크스페이스입니다.  
🌐 웹 UI에서 작업을 보내고, 📡 Telegram과 Discord에서도 같은 에이전트를 이어서 쓸 수 있습니다.  
...
- 💬 Discord slash command 기반 에이전트 실행 지원
...
Discord slash command도 같이 붙일 경우:
...
Codexia는 Discord에서는 slash command 기반으로 동작합니다. Telegram처럼 별도 메시지 파서를 두지 않고, Discord interaction endpoint에서 바로 세션을 이어받습니다.

### Discord로 할 수 있는 것

- `/run message:<요청>` 으로 Codex 작업 실행
- `/run attachment:<파일>` 로 첨부 파일과 함께 실행
- `/status`, `/jobs`, `/recent` 로 현재 세션 상태 확인
- `/new`, `/session`, `/resume` 로 세션 전환
- `/model`, `/effort` 로 기본 모델/사고수준 변경
- `/cancel`, `/help` 지원

### Discord 연결 순서

1. Discord Developer Portal에서 Interaction Endpoint URL을 `/api/discord` 로 설정
2. `DISCORD_PUBLIC_KEY`, `DISCORD_APPLICATION_ID`, `DISCORD_BOT_TOKEN` 환경 변수 설정
3. `pnpm discord:commands` 로 slash command 등록
4. 서버를 띄운 뒤 Discord에서 명령 실행
...
| 변수명 | 필수 | 설명 |
| --- | --- | --- |
| `DISCORD_PUBLIC_KEY` | Discord 사용 시 필수 | Interaction 서명 검증용 공개 키 |
| `DISCORD_APPLICATION_ID` | 명령 등록 시 필수 | Discord 애플리케이션 ID |
| `DISCORD_BOT_TOKEN` | 명령 등록 시 필수 | Slash command 등록용 봇 토큰 |
| `DISCORD_COMMAND_GUILD_ID` | 선택 | 특정 guild에만 빠르게 명령 동기화할 때 사용 |
| `DISCORD_ALLOWED_USER_IDS` | 선택 | 허용할 Discord user id 목록 |
| `DISCORD_ALLOWED_CHANNEL_IDS` | 선택 | 허용할 Discord channel id 목록 |
| `DISCORD_ALLOWED_GUILD_IDS` | 선택 | 허용할 Discord guild id 목록 |
| `DISCORD_SESSION_OVERRIDES_FILE` | 선택 | scope별 현재 세션 선택 상태 저장 파일 경로 |
| `DISCORD_EVENT_LOG_FILE` | 선택 | Discord 이벤트 로그 파일 경로 |
| `DISCORD_FILE_DOWNLOAD_DIR` | 선택 | Discord 첨부 파일 임시 저장 경로 |
...
- `data/discord-session-overrides.json`
- `data/discord-events.log`
- `data/discord-files/`

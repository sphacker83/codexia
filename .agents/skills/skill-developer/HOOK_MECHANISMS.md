...
사용자가 프롬프트를 제출
    ↓
.agents/workflows/settings.json이 훅을 등록
    ↓
...
포맷된 메시지를 stdout으로 출력
    ↓
stdout이 Gemini의 컨텍스트가 됨(프롬프트 전에 주입)
    ↓
Gemini가 봄: [스킬 제안] + 사용자 프롬프트
```

### 핵심 포인트

- **종료 코드(Exit code)**: 항상 0(허용)
- **stdout**: → Gemini 컨텍스트(시스템 메시지로 주입)
- **타이밍**: Gemini가 프롬프트를 처리하기 *이전*에 실행
- **동작**: 비차단, 안내(advisory) 전용
- **목적**: 관련 스킬을 Gemini가 인지하도록 함
...
Gemini는 이 출력을 사용자 프롬프트 처리 전에 추가 컨텍스트로 받습니다.
...
```
Gemini가 Edit/Write 툴을 호출
    ↓
.agents/workflows/settings.json이 훅을 등록(matcher: Edit|Write)
    ↓
...
그 외:
  종료 코드 0으로 종료(ALLOW)
    ↓
차단(BLOCK)된 경우:
  stderr → Gemini가 메시지를 봄
  Edit/Write 툴이 실행되지 않음
  Gemini가 스킬을 사용한 뒤 재시도해야 함
허용(ALLOW)된 경우:
  툴이 정상 실행
```
...
- **종료 코드 2**: 차단(BLOCK) (stderr → Gemini)
- **종료 코드 0**: 허용(ALLOW)
- **타이밍**: 툴 실행 *이전*에 실행
...
Gemini는 이 메시지를 보고, 편집을 재시도하기 전에 스킬을 사용해야 한다고 이해합니다.
...
| 종료 코드(Exit Code) | stdout | stderr | 툴 실행 | Gemini가 보는 것 |
|-----------|--------|--------|----------------|-------------|
| 0 (UserPromptSubmit) | → 컨텍스트 | → 사용자만 | 해당 없음 | stdout 내용 |
| 0 (PreToolUse) | → 사용자만 | → 사용자만 | **진행** | 없음 |
| 2 (PreToolUse) | → 사용자만 | → **GEMINI** | **BLOCKED** | stderr 내용 |
...
1. PreToolUse에서 Gemini에게 메시지를 보낼 수 있는 **유일한 방법**
2. stderr 내용이 Gemini에게 "자동으로 피드백"됨
3. Gemini가 차단 메시지를 보고 다음 행동을 이해함
4. 툴 실행이 방지됨
...
같은 세션에서 반복적으로 잔소리하듯 차단하지 않기 위해, Gemini가 한 번 스킬을 사용하면 다시 차단하지 않도록 합니다.

### 상태 파일 위치

`.agents/workflows/hooks/state/skills-used-{session_id}.json`
...
- Gemini가 스킬을 쓰지 않고 다른 편집을 해도 다시 차단하지 않을 수 있음
- Gemini가 지시를 따를 것이라는 전제를 둠

...
컨텍스트에 따라 자동 활성화되는, 프로덕션에서 검증된 Agent Workflows용 스킬 모음입니다.

---

## 스킬이란?

스킬은 필요할 때 Gemini가 로드하는 모듈형 지식 베이스입니다. 스킬은 다음을 제공합니다:
- 도메인별 가이드라인
- 모범 사례
- 코드 예제
- 피해야 할 안티패턴
...
현재 기본 제공 스킬은 **10개**입니다.


### skill-developer (메타 스킬)
**목적:** Agent Workflows 스킬 생성 및 관리
...
전체 안내는 [AGENT_WORKFLOWS.md](../../AGENT_WORKFLOWS.md)를 참고하세요.

### 수동 통합

**1단계: 스킬 디렉터리 복사**
```bash
cp -r codex-code-infrastructure-showcase/.agents/workflows/skills/backend-dev-guidelines \\
      your-project/.agents/workflows/skills/
```
...
없다면 생성하세요:
```bash
cp codex-code-infrastructure-showcase/.agents/workflows/skills/skill-rules.json \\
   your-project/.agents/workflows/skills/
```
...
**확인:**
1. 스킬 디렉터리가 `.agents/workflows/skills/`에 있나요?
...
# 훅 수동 실행 테스트
./.agents/workflows/hooks/skill-activation-prompt.sh
```
...
**질문이 있나요?** 더 자세한 통합 가이드는 [AGENT_WORKFLOWS.md](../../AGENT_WORKFLOWS.md)를 참고하세요.

...
이 파일은 이 저장소에서 Codex 에이전트가 `.codex` 자산(스킬/훅/에이전트/명령어/Dev Docs)을 최대한 활용하도록 하는 운영 규칙입니다.
...
## 11) Godot UI 스케일 고정 규칙 (DungeonNeko)

- 대상: `dev/active/godot-porting/*`, `DungeonNeko-Godot/scripts/ui/*`
- 월드 확대는 카메라(`DUNGEON_NEKO_CAMERA_ZOOM`)로 처리하고, UI 확대는 카메라/렌더스케일로 처리하지 않는다.
- UI는 **실치수 계산**으로만 키운다. 현재 기준값은 `GameHudController.UiMetricScale = 2.0`.
- 팝업/패널의 **외곽 컨테이너 크기**는 디자인 원본 크기를 유지한다.
- 내부 요소(폰트/아이콘/내부 장식/내부 여백)는 `UiMetricScale` 기준으로 확대한다.
- 금지: `render scale=2` 또는 카메라 배율 변경으로 UI 크기를 맞추는 방식.
- 신규 UI 수정 시 반드시 `godot-porting-context.md`와 `godot-porting-tasks.md`의 UI 스케일 항목을 먼저 확인하고 동일 규칙으로 구현한다.

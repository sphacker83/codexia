...
**세션 상태 확인:**
```bash
ls .codex/hooks/state/
cat .codex/hooks/state/skills-used-{session-id}.json
```
...
```bash
rm .codex/hooks/state/skills-used-{session-id}.json
```
...
```bash
cat <<'EOF' | npx tsx .codex/hooks/skill-verification-guard.ts 2>&1
{
  "session_id": "debug",
  "tool_name": "Edit",
...
**Check `.codex/settings.json`:**
```bash
cat .codex/settings.json | jq '.hooks.UserPromptSubmit'
cat .codex/settings.json | jq '.hooks.PreToolUse'
```
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$CODEX_PROJECT_DIR/.codex/hooks/skill-activation-prompt.sh"
          }
        ]
      }
    ]
...
**확인:**
```bash
head -1 .codex/hooks/skill-activation-prompt.sh
```
...
```bash
# UserPromptSubmit
time echo '{"prompt":"test"}' | npx tsx .codex/hooks/skill-activation-prompt.ts

# PreToolUse
time cat <<'EOF' | npx tsx .codex/hooks/skill-verification-guard.ts

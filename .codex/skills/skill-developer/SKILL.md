...
description: Anthropic 모범 사례에 따라 Roo Codex 스킬을 만들고 관리합니다. 새 스킬 생성, skill-rules.json 수정, 트리거 패턴 이해, 훅 작업, 스킬 활성화 디버깅, 점진적 공개(progressive disclosure) 구현 시 사용하세요. 스킬 구조, YAML 프론트매터, 트리거 유형(키워드, 의도 패턴, 파일 경로, 콘텐츠 패턴), enforcement 레벨(block/suggest/warn), 훅 메커니즘(UserPromptSubmit/PreToolUse), 세션 추적, 500줄 규칙을 다룹니다.
---
...
자동 활성화 시스템이 있는 Roo Codex에서 스킬을 만들고 관리하기 위한 포괄 가이드입니다. 500줄 규칙과 점진적 공개(progressive disclosure) 패턴을 포함한 Anthropic 공식 모범 사례를 따릅니다.
...
- **파일**: `.codex/hooks/skill-activation-prompt.ts`
- **트리거**: Codex가 사용자 프롬프트를 보기 **이전**
- **목적**: 키워드 + 의도 패턴을 기반으로 관련 스킬 제안
- **방식**: 포맷된 리마인더를 컨텍스트로 주입(stdout → Codex 입력)
- **사용 사례**: 주제 기반 스킬, 암묵적 작업 감지

**2. Stop 훅 - 에러 처리 리마인더** (부드러운 리마인더)
- **파일**: `.codex/hooks/error-handling-reminder.ts`
- **트리거**: Codex가 응답을 마친 **이후**
...
**위치**: `.codex/skills/skill-rules.json`
...
**위치:** `.codex/skills/{skill-name}/SKILL.md`
...
- 훅에서 종료 코드 2를 반환, stderr → Codex
- Codex가 차단 메시지를 보고, 진행하려면 스킬을 사용해야 함
...
- Codex가 프롬프트를 보기 전에 리마인더를 주입
- Codex가 관련 스킬을 인지하도록 함
- 강제는 아니고 안내만 제공
...
**상태 파일:** `.codex/hooks/state/skills-used-{session_id}.json`
...
- [ ] `.codex/skills/{name}/SKILL.md`에 스킬 파일 생성
...
1. 프론트매터를 포함해 `.codex/skills/{name}/SKILL.md` 생성
2. `.codex/skills/skill-rules.json`에 엔트리 추가
3. `npx tsx` 명령으로 테스트
...
**설정(Configuration):**
- `.codex/skills/skill-rules.json` - 마스터 설정
- `.codex/hooks/state/` - 세션 추적
- `.codex/settings.json` - 훅 등록

**훅(Hooks):**
- `.codex/hooks/skill-activation-prompt.ts` - UserPromptSubmit
- `.codex/hooks/error-handling-reminder.ts` - Stop 이벤트(부드러운 리마인더)

**모든 스킬(All Skills):**
- `.codex/skills/*/SKILL.md` - 스킬 콘텐츠 파일
